...
export default function Home() {
  ...
  return (
    <div className="container px-4 md:mx-auto md:px-4 lg:px-0 max-w-5xl overflow-x-none">
      <Head>
        <title>DETA {detaBaseName && "- " + detaBaseName}</title>
        <link rel="icon" href="/logo.svg" />
      </Head>
      <div className={"py-8 " + (popupOpened && " blur-[2px] py-8 animate-pulse")}>
        <div className="flex flex-row mb-1 sm:mb-0 justify-between w-full">
          <div className="flex justify-center my-auto items-center max-h-6">
        ...
        <div className="xl:px-0 py-4">
          <div className="text-sm block shadow rounded-lg overflow-x-auto">
            {detaBaseContent.length > 0 &&
                    ...
                    return <React.Fragment key={index}>
                      {item.key.includes(Search) &&
                        <tr key={index + "key"}>
                          ...
                          <td className="md:px-5 py-5 border-b border-gray-200 bg-white">
                                  ...
                                  <div className={`flex items-center ${Array.isArray(item[key]) && "justify-center"} ${typeof item[key] === "boolean" && "justify-center"}`}>
                                    {
                                      typeof item[key] === 'boolean' ?
                                        <>
                                          <input readOnly={true} checked={item[key]} type="checkbox" className="hover:cursor-pointer relative appearance-none h-6 w-6 border border-gray-300 rounded-md checked:bg-pink-500 checked:border-transparent focus:outline-none" />
                                          <svg onClick={() => {
                                        ...
                                        <p onClick={() => {
                                          setPopupContent({ item: item, key: key, value: item[key] })
                                          setPopupOpened(true)
                                        }} title={Array.isArray(item[key]) ? "array" : item[key]} className={`text-gray-900 truncate max-w-[5rem] min-w-full ${Array.isArray(item[key]) && "px-2 min-w-min rounded-md hover:cursor-pointer hover:border-pink-400 hover:text-pink-400 py-1 border border-slate-400 text-slate-400"}`}>
                                          {Array.isArray(item[key]) ? "array" : (item[key] ? item[key] : "⠀")}
                                        </p>
                                    }
                                  </div>
                                </td>
                ...
                <div className={`relative flex flex-col ` + ((typeof popupContent.value === "boolean") ? "min-h-[15vh]" : "min-h-[35vh]")} >
                  {typeof popupContent.value == "boolean" ?
                    <>
                      <div className='w-full max-h-32 h-full flex-grow min-h-16 bg-slate-100 focus:border-pink-500 focus:ring-pink-500 border-2 border-gray-300 rounded-lg shadow-sm p-4 text-gray-700 resize-none'>
                        ...
                        <div className="flex my-1 space-x-2">
                          ...
                          } className="relative appearance-none h-6 w-6 border border-gray-300 rounded-md checked:bg-pink-
                          500 checked:border-transparent focus:outline-none" />
                        </div>
                    ...
                    : <>
                      ...
                      }} readOnly={false} defaultValue={Array.isArray(popupContent.value) ? (popupContent.value.map(
                        (item, index) => {
                          return JSON.stringify(item, null, 2)
                        }
                      )) : popupContent.value} className="w-full max-h-56 flex-grow min-h-16 bg-slate-100 focus:border-pink-500 focus:ring-pink-500 border-2 border-gray-300 rounded-lg shadow-sm p-4 text-gray-700 resize-none" placeholder="Enter your content here" />
